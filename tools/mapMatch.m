...
if ischar(seedMets)
    if strcmpi(seedMets,'noSeed') % if matching seeds are not provided
        ...
        diffNumRxns = abs(repmat((Trxnwithmet ./ metInModelSum)',length(Cmodel.mets),1)  - repmat(Crxnwithmet,1,length(Tmodel.mets) ) ) ;
        % difference in number of metabolites that are involved in each reaction
        diffNumMets = abs(repmat(Tmetinrxn',length(Cmodel.rxns),1)  - repmat(Cmetinrxn,1,length(Tmodel.rxns) ) ) ;
                ...
                if  max(mmatch(nowmpos1,:)) > 0
                    ...
                    for iTmodel = 1:length(nowmpos2a)
                        ...
                        if length(rx1) == length(rx2)
                            ...
                            rmatch(rx1,rx2) = ( rmatch(rx1,rx2) + 1./(1+abs( repmat(Cmetinrxn(rx1),1,length(rx2)) - repmat(Tmetinrxn(rx2)',length(rx1),1) )) ) ;
                ...
                if  max(rmatch(nowrpos1,:)) > 0
                    ...
                    for ir2 = 1:length(nowrpos2a)
                        ...
                        if length(met1) == length(met2)
                            ...
                            mmatch(met1,met2) = ( mmatch(met1,met2) + 1./(1+abs( repmat(Crxnwithmet(met1),1,length(met2)) - repmat(Trxnwithmet(met2)',length(met1),1) )) ) ;
