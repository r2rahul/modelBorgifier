...
% Check to see if cost and gamma already exist.
if isfield(handles.M.Stats,'cost') && isfield(handles.M.Stats,'gamma')
    handles.cost = handles.M.Stats.cost ;
    handles.gamma = handles.M.Stats.gamma ;
else
    % Placeholder until parameter optimizer is run.
    handles.cost = 25 ; 
    handles.gamma = 0 ; 
end
...
Stats.cost = handles.cost ;
Stats.gamma = handles.gamma ; 
varargout{3} = Stats ; 
...
h = findobj(gcf,'Enable','on') ;
set(h,'Enable','off')
if get(handles.checkbox_optParams,'Value') 
    handles.M.Stats = ...
        optimalScores(handles.rxnList,'svm',1) ;
    handles.cost = handles.M.Stats.cost ; 
    handles.gamma = handles.M.Stats.gamma ;
    handles.M.scoreTotal = handles.M.Stats.scoreTotal ;
else
    handles.M.Stats = ...
        optimalScores(handles.rxnList,'svm',handles.cost,handles.gamma) ;  
    handles.M.scoreTotal = handles.M.Stats.scoreTotal ;
end
set(h,'Enable','on') 
fillStats(handles)
...
function checkbox_optParams_Callback(hObject, eventdata, handles)
end
